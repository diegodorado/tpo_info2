...
typedef struct
{
  union
  {
    /* D/A Converter Register. This register contains the digital value to be R/W 0 converted to analog and a power control bit. */__RW uint32_t DACR;

    /* Table 539: D/A Converter Register (DACR - address 0x4008 C000) bit description */

    struct
    {
      /* Reserved, user software should not write ones to reserved bits. The value read from a NA reserved bit is not defined. */__R uint32_t RESERVED0 :6;
...
typedef struct
{
  __RW uint32_t CR0;   // Control Register 0. Selects the serial clock rate, bus type, and data size.
  __RW uint32_t CR1;   // Control Register 1. Selects master/slave and other modes
  __RW uint32_t DR;    // Data Register. Writes fill the transmit FIFO, and reads empty the receive FIFO.
  __R  uint32_t SR;    // Status Register
  __RW uint32_t CPSR;  // Clock Prescale Register
  __RW uint32_t IMSC;  // Interrupt Mask Set and Clear Register
  __RW uint32_t RIS;   // Raw Interrupt Status Register
  __RW uint32_t MIS;   // Masked Interrupt Status Register
  __RW uint32_t ICR;   // Interrupt Clear Register
  __RW uint32_t DMACR; // DMA Control Register
} ssp_register_t;


#define  SSP0    ( ( ssp_register_t  * ) 0x40088000UL )
#define  SSP1    ( ( ssp_register_t  * ) 0x40030000UL )
